...
#        x <- apply(permutations(n = length(p), r = i, v = p),1, paste, collapse='+')
            ...
            write.csv(x, file=paste('D:/CRTI/r_projects/ModelPredictorRanking/output/all_model_formulas_', i, '.csv', sep=''), row.names=FALSE)
      }
}
...
concat_files <- function()
{
      ...
      for (i in seq(min_predictors,max_predictors))
      {
            y <- read.csv (paste('D:/CRTI/r_projects/ModelPredictorRanking/output/all_model_formulas_', i, '.csv', sep=''), as.is = TRUE, header = TRUE)
            x <- rbind(x,y)
      }
      ...
      {
            write.csv(x[[i]], file=paste('D:/CRTI/r_projects/ModelPredictorRanking/output/all_model_formulas_P', i, '.csv', sep=''), row.names=FALSE)
      }
}
...
build_models <- function(species='Acer saccharinum') 
{
      ...
      dir.create(file.path('D:/CRTI/r_projects/ModelPredictorRanking/output/', species), showWarnings = FALSE)
...
#      for (x in p) {
            ...
            formulas = read.csv (paste('D:/CRTI/r_projects/ModelPredictorRanking/output/all_model_formulas_P', x, '.csv', sep=''), as.is = TRUE, header = TRUE)
            ...
            for (j in seq(1, nrow(formulas), chunk_size))
            {
                  ...
                  write.table(aics, file=paste('D:/CRTI/r_projects/ModelPredictorRanking/output/', species, '/all_model_AICs_', x, '.csv', sep=''), sep=',', col.names=FALSE, row.names=FALSE, append=TRUE)
                  aics = NULL
            }
      }
      stopCluster(cl)
...
analyze_results <- function (species='Acer saccharinum')
{
      ...
      for (i in seq(1:threads))
      {
            r <- read.csv (paste('D:/CRTI/r_projects/ModelPredictorRanking/output/', species, '/all_model_AICs_', i, '.csv', sep=''), as.is = TRUE, header = FALSE) 
            names(r) <- c("index", "aic")
            r <- r[complete.cases(r),]
            r <- cbind (partition=i, r)
            
            f = read.csv (paste('D:/CRTI/r_projects/ModelPredictorRanking/output/all_model_formulas_P', i, '.csv', sep=''), as.is = TRUE, header = TRUE)
            names(f) <- c("formula")
            f <- head(f,nrow(r))
      ...
      write.table(ordered_results, file=paste('D:/CRTI/r_projects/ModelPredictorRanking/output/', species, '/all_model_AICs_ordered.csv', sep=''), sep=',', col.names=TRUE, row.names=FALSE)
...
{
      a <- read.csv (paste('D:/CRTI/r_projects/ModelPredictorRanking/output/', species, '/all_model_AICs_ordered.csv', sep=''), as.is = TRUE, header = TRUE)
      scorecard <- numeric()
      for (pred in p)
      {
      ...
      write.table(scorecard, file=paste('D:/CRTI/r_projects/ModelPredictorRanking/output/scorecard.csv',sep=''), sep=',', col.names=TRUE, row.names=TRUE)
      return (scorecard)
...
report_common_species <- function()
{
      ...
      write.table(report_header, file='D:/CRTI/r_projects/ModelPredictorRanking/output/common_species_aic.csv', sep=',', col.names=FALSE, row.names=FALSE)
      write.table(report_header, file='D:/CRTI/r_projects/ModelPredictorRanking/output/common_species_rank.csv', sep=',', col.names=FALSE, row.names=FALSE)
      ...
      {
            cat("Processing ",spp, "\n")
            ifelse(!dir.exists(paste('D:/CRTI/r_projects/ModelPredictorRanking/output/', spp, sep='')), dir.create(paste('D:/CRTI/r_projects/ModelPredictorRanking/output/', spp, sep='')), FALSE)
            build_models(spp)
            analyze_results(spp)
            s1 <- round(score_predictors(spp),0)
            s1_ordered <- t(s1[order(names(s1))])
            write.table(cbind(species=spp, s1_ordered), file='D:/CRTI/r_projects/ModelPredictorRanking/output/common_species_aic.csv', sep=',', col.names=FALSE, row.names=FALSE, append=TRUE)
            ...
            write.table(cbind(species=spp,s1_ranked), file='D:/CRTI/r_projects/ModelPredictorRanking/output/common_species_rank.csv', sep=',', col.names=FALSE, row.names=FALSE, append=TRUE)
      }
}
