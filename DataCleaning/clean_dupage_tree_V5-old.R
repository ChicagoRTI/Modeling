...
chicago_tree_data = read.csv ('D:/CRTI/data/SurveyNov2017_canopy_dist.txt', as.is = TRUE, header = TRUE, sep='\t')
...
#      chicago_tree_data[[lu_name_col]] <- chicago_tree_data[[lu_name_col]] + 1
      ...
      HEIGHT_MAX        <- c(0, 150)
      HEIGHT_MEAN       <- c(0, 100)
      limits            <- rbind (limits, CROWN_AREA, CROWN_COMPACTNESS, DBH_IN, HEIGHT_MAX, HEIGHT_MEAN)
      ...
      for (v in c('CROWN_AREA', 'CROWN_COMPACTNESS', 'DBH_IN', 'HEIGHT_MAX', 'HEIGHT_MEAN'))
      {
      ...
      # Add 3 new columns (with zero values)
      ctree['NDVI_MEAN'] <- 0L
      ctree['NDVI_MAX'] <- 0L
      ctree['NDVI_STD'] <- 0L
