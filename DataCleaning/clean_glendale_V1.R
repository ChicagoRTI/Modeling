# source("D:/CRTI/r_projects/data_cleaning/clean_glendale_V1.r")
# clean()

# Read the CSV with the tree data 
chicago_tree_data = read.csv ('D:/CRTI/data/Glendale_test_area_model_ready.csv', as.is = TRUE, header = TRUE, sep=',')
...
#      chicago_tree_data[[lu_name_col]] <- chicago_tree_data[[lu_name_col]] + 1
      ...
      write.csv(ctree, file='D:/CRTI/data/cleaned/glendale_accepted_V1.csv', row.names=FALSE)
      write.csv(rejects, file='D:/CRTI/data/cleaned/glendale_rejected_V1.csv', row.names=FALSE)
}
